...
#define kYJPageTabbarHeight 49

@interface YJPageScrollViewController ()<UIScrollViewDelegate, YJPageScrollViewMenuDelegate, UITableViewDelegate, YJPageScrollHeaderViewDelegate> {
...
/** headerView*/
@property (nonatomic, strong) YJPageScrollHeaderView *bigHeaderView;
...
@implementation YJPageScrollViewController
    ...
    if (!self.scrollViewMenu) {
        ...
        if (_isAsChildViewController && _isAfterLoadData) {
            if (self.navigationController.navigationBar.isTranslucent) {
                CGFloat deltaHeight =  self.configration.showNavigation ? [self _navgaitonHeight] : 0;
                CGFloat dHeight  =  (self.configration.showTabbar ? kYJPageTabbarHeight : 0);
                self.view.yj_y =self.parentViewController.edgesForExtendedLayout != UIRectEdgeNone ?  deltaHeight : 0;
                self.view.yj_height =self.view.yj_height -  deltaHeight - dHeight;
            }else{
                CGFloat deltaHeight =  self.configration.showNavigation ? [self _navgaitonHeight] : 0;
                CGFloat dHeight  =  (self.configration.showTabbar ? kYJPageTabbarHeight : 0);
                self.view.yj_height =self.view.yj_height -  deltaHeight - dHeight;
        ...
        self.parentScrollView.frame = CGRectMake(0, [self isTopStyle] ? self.configration.menuHeight : 0, self.view.yj_width, ([self isTopStyle] ? self.view.yj_height - self.configration.menuHeight: self.view.yj_height));
        
        self.parentScrollView.contentSize = CGSizeMake(self.view.yj_width * self.viewControllers.count, self.view.yj_height - ([self isTopStyle] ? self.configration.menuHeight : 0));
        
        if ([self isSuspensionStyle]) {
            self.bigHeaderView.frame = self.headerView.frame;
            self.bigHeaderView.yj_height += self.configration.menuHeight;
            [self.bigHeaderView addSubview:self.headerView];
            
            if (self.HeaderViewCouldScale) {
                self.scaleBackgroundView.hidden = YES;
                self.scaleBackgroundView.frame = CGRectMake(0, 0, self.headerView.yj_width, self.headerView.yj_height);
            }
            
            [self initPageScrollViewMenuWithFrame:CGRectMake(0, self.headerView.yj_height, self.view.yj_width, self.configration.menuHeight)];
            
            self.originHeaderOffSetY = self.scrollViewMenu.yj_y;
            
            UITableView *tableView = (UITableView *)[self getScrollViewForDataSource];
            tableView.tableHeaderView = self.bigHeaderView;
            tableView.scrollIndicatorInsets = UIEdgeInsetsMake(self.bigHeaderView.yj_height, 0, 0, 0);
            
            ((YJPageScrollView *)self.parentScrollView).headerViewHeight = self.bigHeaderView.yj_height;
...
- (void)viewDidLayoutSubviews{
    ...
    if (!self.DidLayoutSubviews && [self isNavigationStyle]) {
        ...
        __block CGFloat menuX = self.view.yj_x;
        ...
        CGFloat menuWidth = self.view.yj_width - menuX - rightWidth;
        [self initPageScrollViewMenuWithFrame:CGRectMake(menuX, self.view.yj_y, menuWidth, menuHeight)];
    }
}

- (void)initPageScrollViewMenuWithFrame:(CGRect)frame{
    
    YJPageScrollViewMenu *scrollViewMenu = [YJPageScrollViewMenu pageScrollViewMenuWithFrame:frame titles:self.titleArrayM Configration:self.configration delegate:self currentIndex:self.pageIndex];
...
- (void)addViewControllerToParentScrollView:(UIViewController *)viewController index:(NSInteger)index{
    ...
    viewController.view.frame = CGRectMake(self.view.yj_width * index, 0, self.parentScrollView.yj_width, self.parentScrollView.yj_height);
    ...
    if (![viewController isKindOfClass:[UITableViewController class]]) {
        scrollView.frame = CGRectMake(0, 0, self.parentScrollView.yj_width, self.parentScrollView.yj_height);
    }
    ...
    if (![self.cacheDictionaryM objectForKey:title]) {//缓存
        ...
        if ([self isSuspensionStyle]) {
            ...
            if (self.placeHoderView) {
                self.placeHoderView.yj_height = self.view.yj_height;
    ...
    if (newValue != oldValue){
        ...
        if (self.HeaderViewCouldScale) {//是否是伸缩效果
            ...
            if(yOffset < 0) {
                ...
                if (self.headerViewScaleMode == YJHeaderViewScaleModeTop) {
                    
                    
                    f.origin.y = 0;
                    f.size.height =  -yOffset + self.headerView.yj_height;
                    f.origin.x = xOffset;
                    f.size.width = self.view.yj_width + fabs(xOffset)*2;
                    
                    self.tempHeaderView.yj_y = -yOffset;
                    
                    
                }else {
                    f.origin.y = yOffset;
                    f.size.height =  -yOffset*2 + self.headerView.yj_height;
                    f.origin.x = xOffset;
                    f.size.width = self.view.yj_width + fabs(xOffset)*2;
                }
        ...
        CGFloat progress = 0;
        CGFloat deltaHeight =  self.configration.showNavigation ? 0 : [self _navgaitonHeight];
        
        if (y <= deltaHeight) {
            
            NSString *title = [self titleWithIndex:self.pageIndex];
            progress = 1;
            self.scrollViewMenu.yj_y = deltaHeight;
            [self.contentOffsetDictionaryM setObject:@(newValue) forKey:title];
            
        }else{
            
            progress = 1-((y-deltaHeight)/(self.headerView.yj_height - deltaHeight));
            self.scrollViewMenu.yj_y = y;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    CGFloat offsetX = currentPostion / self.view.yj_width;
...
//    [self.scrollViewMenu adjustItemPositionWithCurrentIndex:ceilf(offsetX)];
}
...
#pragma mark - YJPageScrollViewMenuDelegate
...
//    [self.parentScrollView setContentOffset:CGPointMake(frame.origin.x, frame.origin.y) animated:YES];
...
- (void)replaceTempHeaderView{
    ...
    if (!self.isHeaderViewInTempHeaderView && [self isSuspensionStyle]) {
        ...
        self.isHeaderViewInTempHeaderView = YES;
        self.tempHeaderView.yj_y = [self.headerView convertRect:self.headerView.frame toView:self.view].origin.y;
...
- (void)updateSubViewScrollViewContentInset{
    ...
    CGFloat deltaHeight =  self.configration.showNavigation ? 0 : [self _navgaitonHeight];
    if (self.scrollViewMenu.yj_y  == deltaHeight) {//临界点
...
#pragma mark - YJPageScrollHeaderViewDelegate
...
+ (instancetype)pageScrollViewControllerWithControllers:(NSArray *)viewControllers titles:(NSArray *)titleArrayM Configration:(YJPageScrollViewMenuTool *)configration{
    ...
    YJPageScrollViewController *viewController =  [[self alloc]init];
    viewController.viewControllers = viewControllers.mutableCopy;
    viewController.titleArrayM = titleArrayM.mutableCopy;
    viewController.configration = configration ? configration : [YJPageScrollViewMenuTool pageScrollViewMenuTool];
...
- (void)reloadYJPageScrollViewControllerRemoveCacheLoadPage:(NSNumber *)index {
    ...
    [self reloadYJPageScrollViewControllerLoadPage:index];
}
...
- (void)reloadPlaceHoderViewFrame{
    ...
    if (self.placeHoderView && [self isSuspensionStyle]) {
        UITableView *tableView = (UITableView *)[self getScrollViewForDataSource];
        CGFloat deltaHeight =  self.configration.showNavigation ? 0 : [self _navgaitonHeight];
        if (tableView.contentSize.height > self.parentScrollView.yj_height) {
            
            CGFloat footerHeight = (tableView.tableFooterView == nil ? 0 : self.placeHoderView.yj_height) ;
            
            CGFloat height = tableView.contentSize.height - footerHeight - self.headerView.yj_height;
            if (height > self.parentScrollView.yj_height) {
                
                tableView.tableFooterView = nil;
            }else{
                
                self.placeHoderView.yj_height = self.parentScrollView.yj_height - height  -deltaHeight;
                tableView.tableFooterView = self.placeHoderView;
            }
        }else{
            
            self.placeHoderView.yj_height = self.parentScrollView.yj_height - tableView.contentSize.height + self.headerView.yj_height -deltaHeight;
            tableView.tableFooterView = self.placeHoderView;
        }
    }
}
...
- (BOOL)isTopStyle{
    return self.configration.pageScrollViewMenuStyle == YJPageScrollViewMenuStyleTop;
}

- (BOOL)isNavigationStyle{
    return self.configration.pageScrollViewMenuStyle == YJPageScrollViewMenuStyleNavigation;
}

- (BOOL)isSuspensionStyle{
    return self.configration.pageScrollViewMenuStyle == YJPageScrollViewMenuStyleSuspension;
}
...
- (UIScrollView *)parentScrollView{
    ...
    if (!_parentScrollView) {
        _parentScrollView = [[YJPageScrollView alloc]init];
...
- (UIView *)tempHeaderView{
    if (!_tempHeaderView) {
        _tempHeaderView = [[UIView alloc]initWithFrame:CGRectMake(0, [self _navgaitonHeight], self.view.yj_width, 0)];
...
- (UIView *)bigHeaderView{
    ...
    if (!_bigHeaderView) {
        _bigHeaderView = [[YJPageScrollHeaderView alloc] init];
    ...
    if (![self.scrollViewCacheDictionryM objectForKey:title]) {
        ...
        if (self.dataSource && [self.dataSource respondsToSelector:@selector(pageScrollViewController:scrollViewForIndex:)]) {
            ...
            if (scrollView && [scrollView isKindOfClass:[UIScrollView class]]) {
                if (self.configration.pageScrollViewMenuStyle == YJPageScrollViewMenuStyleSuspension) {
